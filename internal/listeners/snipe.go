...
func (s *SnipeCache) TrackMessage(m *discordgo.Message) {
	...
	avatarURL := helpers.UserAvatar(m.Author)
	if m.Member != nil && m.Member.Avatar != "" {
		avatarURL = m.Member.AvatarURL("256")
	}
...
func OnMessageReactionRemove(s *discordgo.Session, r *discordgo.MessageReactionRemove) {
	...
	if member, err := helpers.GetGuildMember(s, r.GuildID, r.UserID); err == nil && member != nil && member.User != nil {
		...
		avatarURL = helpers.UserAvatar(member.User)
...
func OnMessageDelete(s *discordgo.Session, m *discordgo.MessageDelete) {
	...
	if m.BeforeDelete != nil && m.BeforeDelete.Author != nil {
		...
		authorAvatar = helpers.UserAvatar(m.BeforeDelete.Author)
		if m.BeforeDelete.Member != nil && m.BeforeDelete.Member.Avatar != "" {
			authorAvatar = m.BeforeDelete.Member.AvatarURL("256")
		}
...
func OnMessageUpdate(s *discordgo.Session, m *discordgo.MessageUpdate) {
	...
	if m.BeforeUpdate != nil && m.BeforeUpdate.Author != nil {
		...
		authorAvatar = helpers.UserAvatar(m.BeforeUpdate.Author)
		if m.BeforeUpdate.Member != nil && m.BeforeUpdate.Member.Avatar != "" {
			authorAvatar = m.BeforeUpdate.Member.AvatarURL("256")
		}
	...
	if authorID == "" && s != nil {
		if msg, err := s.ChannelMessage(m.ChannelID, m.ID); err == nil && msg != nil && msg.Author != nil {
			...
			if msg.Member != nil && msg.Member.Avatar != "" {
				authorAvatar = msg.Member.AvatarURL("256")
			}
	...
	if authorID == "" || isBot || oldContent == m.Content || (oldContent == "" && m.Content == "") || IsAutoModDeleted(m.ID) {
		return
	}
	...
	GlobalSnipeCache.AddEdited(m.GuildID, m.ChannelID, entry)
	...
	if DB != nil && m.GuildID != "" {
		...
		modlog.Log(s, DB, &modlog.MessageEditEvent{
			GuildID:      m.GuildID,
			Author:       authorUser,
			AuthorAvatar: authorAvatar,
			ChannelID:    m.ChannelID,
